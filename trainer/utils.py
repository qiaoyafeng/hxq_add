    ...
    for mode in ["train", "validation", "test"]:
        if mode == "test":
            ...
            dataset = DepressionDataset(
                data_config[f"{mode}_ROOT_DIR".upper()],
                mode,
...
def find_last_ckpts(path, key, date=None):
    ...
    if date is not None:
        ...
        assert (
            matched
        ), "The given date is the incorrect date string format. It should be YYYY-MM-DD"
...
def get_models(model_config, args, model_type=None, ckpt_path=None):
    ...
    """
    visual_net = ConvLSTMVisual(
        input_dim=model_config["VISUAL_NET"]["INPUT_DIM"],
    ...
    if model_config["WEIGHTS"]["TYPE"].lower() == "last":
        ...
        weights_path = find_last_ckpts(
            path=ckpt_path, key=model_type, date=model_config["WEIGHTS"]["DATE"]
        )

    elif model_config["WEIGHTS"]["TYPE"].lower() == "absolute_path":
        assert (
            model_config["WEIGHTS"]["CUSTOM_ABSOLUTE_PATH"] is not None
        ), "'CUSTOM_ABSOLUTE_PATH' (absolute path to wights file) in config file under 'WEIGHTS' must be given"
        assert os.path.isabs(
            model_config["WEIGHTS"]["CUSTOM_ABSOLUTE_PATH"]
        ...
        assert (
            model_config["WEIGHTS"]["NAME"] is not None
        ), "'NAME' (name of the wights file) in config file under 'WEIGHTS' must be given"
        weights_path = os.path.join(
            model_config["WEIGHTS"]["PATH"], model_config["WEIGHTS"]["NAME"]
        )
    ...
    if weights_path is not None:
        model_config["WEIGHTS"]["INCLUDED"] = [
            x.lower() for x in model_config["WEIGHTS"]["INCLUDED"]
        ]
...
def get_crossentropy_weights_whole_data(data_config, evaluator_config):
    ...
    if evaluator_config["PREDICT_TYPE"] == "phq-subscores":
        ...
        raise AssertionError(
            "Unknown 'PREDICT_TYPE' for evaluator!", evaluator_config["PREDICT_TYPE"]
        )
...
def get_crossentropy_weights(gt, evaluator_config):
    if evaluator_config["PREDICT_TYPE"] == "phq-subscores":
        ...
        labels, counts = np.unique(gt, return_counts=True)
        for i in range(len(labels)):
            weights[int(labels[i])] = 1.0 / counts[i]
...
def get_criterion(criterion_config, args):
    if criterion_config["USE_SOFT_LABEL"]:
        ...
        if criterion_config["USE_WEIGHTS"]:
            ...
            weights = (
                torch.tensor(criterion_config["WEIGHTS"])
...
def compute_score(probs, evaluator_config, args):
    if evaluator_config["PREDICT_TYPE"] == "phq-subscores":
        ...
        subscores_pred = (
            torch.stack([prob.argmax(dim=-1) * factor for prob in probs], dim=1)
            .sort()[0]
            .to(int)
            .to(float)
        )  # (number of batch, num_subscores)
...
def convert_soft_gt(gt, evaluator_config):
    if evaluator_config["PREDICT_TYPE"] == "phq-subscores":
        ...
        factor = (evaluator_config["N_CLASSES"] - 1) / (
            evaluator_config["CLASSES_RESOLUTION"] - 1
        )
        tmp = [
            stats.norm.pdf(
                np.arange(evaluator_config["CLASSES_RESOLUTION"]),
                loc=score / factor,
                scale=evaluator_config["STD"],
            ).astype(np.float32)
            for score in gt
        ]
...
def compute_loss(criterion, probs, gt, evaluator_config, args, use_soft_label=False):
    if use_soft_label:  # in this case, criterion should be nn.KLDivLoss()
        ...
        if evaluator_config["PREDICT_TYPE"] == "phq-subscores":
            loss = sum(
                [
                    criterion(torch.log(probs[i]), soft_gt[:, i].to(args.device))
        ...
        if evaluator_config["PREDICT_TYPE"] == "phq-subscores":
            ...
            loss = criterion(
                pred_prob.permute(0, 2, 1).contiguous(),
