...
def main(
    ...
    if not config["CRITERION"]["USE_SOFT_LABEL"]:
        assert (
            config["EVALUATOR"]["CLASSES_RESOLUTION"]
            == config["EVALUATOR"]["N_CLASSES"]
    ...
    optimizer, scheduler = get_optimizer_scheduler(
        model_parameters, config["OPTIMIZER"], config["SCHEDULER"]
    )
    ...
    for epoch in range(config["EPOCHS"]):
        log_and_print(
            base_logger,
            f"Epoch: {epoch}  Current Best Test for f1-score: {test_best_f1_score} at epoch {test_epoch_best_f1}",
        )
        log_and_print(
            base_logger,
            f"Epoch: {epoch}  Current Best Test for accuracy: {test_best_acc} at epoch {test_epoch_best_acc}",
        )
        ...
        for mode in ["train", "test"]:
            ...
            for data in tqdm(dataloaders[mode]):
                ...
                def model_processing(input):
                    # print(f"model_processing input: {input}")
                ...
                if mode == "train":
                    ...
                    if config["CRITERION"]["USE_WEIGHTS"]:
                        config["CRITERION"]["WEIGHTS"] = get_crossentropy_weights(
                            gt, config["EVALUATOR"]
                        )
                    ...
                    if config["OPTIMIZER"]["USE_SAM"]:
                        ...
                        for model in models:
                            enable_running_stats(
                                model
                            )  # to avoid potentially problems if you use batch normalization
                ...
                phq_binary_pred.extend(
                    [pred_score[i].item() for i in range(batch_size)]
                )
                ...
                if mode == "train":
                    ...
                    if batch_number % log_interval == 0 and batch_number > 0:
                        ...
                        ms_per_batch = (
                            (time.time() - batches_start_time) * 1000 / log_interval
                        )
                        current_loss = log_interval_loss / log_interval
                        print(
                            f"| epoch {epoch:3d} | {mode} | {batch_number:3d}/{n_batches:3d} batches | "
            ...
            # confusion matrix
            [[tp, fp], [fn, tn]] = standard_confusion_matrix(
                phq_binary_gt, phq_binary_pred
            )
            msg = (
                f"  - Confusion Matrix:\n"
                "    -----------------------\n"
            ...
            tpr, tnr, precision, recall, f1_score = get_classification_scores(
                phq_binary_gt, phq_binary_pred
            )
            msg = (
                "  - Classification:\n"
                ...
                "      Recall: {3:6.4f}\n"
                "      F1-score: {4:6.4f}"
            ).format(tpr, tnr, precision, recall, f1_score)
            log_and_print(base_logger, msg)
            ...
            rho, p = stats.spearmanr(phq_binary_gt, phq_binary_pred)
            msg = (
            ...
            # tensorboard
            writer.add_scalars(
                f"Classification/{mode}/TPR_TNR", {"TPR": tpr, "TNR": tnr}, epoch
            )
            writer.add_scalars(
                f"Classification/{mode}/Scores",
        ...
        if test_model_f1_score >= test_best_f1_score:
            ...
            if args.save:
                ...
                file_path = os.path.join(
                    ckpt_path,
                    "{}_{}_f1_score-{:6.4f}.pt".format(
                        model_type, timestamp, test_best_f1_score
                    ),
                )
        ...
        if test_model_acc >= test_best_acc:
            ...
            if args.save:
                ...
                file_path = os.path.join(
                    ckpt_path,
                    "{}_{}_acc-{:6.4f}.pt".format(model_type, timestamp, test_best_acc),
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        default="config/config_binary.yaml",
    )
    parser.add_argument(
        "--device",
        type=str,
