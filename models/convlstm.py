...
class ConvBlock1d(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, kernel, stride, pad, normalisation, dil=1
    ):
...
class ConvBlock2d(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, kernel, stride, pad, normalisation, att=None
    ):
...
class ConvLSTMVisual(nn.Module):
    def __init__(
        self,
        input_dim,
        output_dim,
        ...
        super(ConvLSTMVisual, self).__init__()
    ...
    def forward(self, net_input):
        ...
        x, _ = self.lstm(
            x
        )  # output shape: (batch, width//stride(pool), lstm_hidden*2) 5x600x128
        x = self.fc(x[:, -1, :].reshape(batch, -1))  # output shape: (batch, output_dim)

        return x


class ConvLSTMAudio(nn.Module):
    def __init__(
        self,
        input_dim,
        output_dim,
        ...
        super(ConvLSTMAudio, self).__init__()
        self.conv = ConvBlock1d(
            in_channels=input_dim,  # 80
        ...
        self.fc = FullyConnected(
            ...
            normalisation=norm,
        )  # ['bn', 'wn']: nn.BatchNorm1d, nn.utils.weight_norm
